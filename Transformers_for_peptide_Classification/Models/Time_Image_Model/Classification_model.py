...
class MultiModal_TimeSeriesImage_Classification(pl.LightningModule):
    def __init__(self, hp, hp_lr, model_name, encoder=None, PrepPatches_raw=None, PrepPatches_image=None):
        ...
        self.loss_cross_entropy= nn.CrossEntropyLoss(label_smoothing=hp["label_smoothing"])

        self.train_accuracy = torchmetrics.classification.Accuracy(task="multiclass", num_classes = num_classes, average='macro')
        self.val_accuracy = torchmetrics.classification.Accuracy(task="multiclass", num_classes = num_classes, average='macro')
        self.test_accuracy_macro = torchmetrics.classification.Accuracy(task="multiclass", num_classes = num_classes, average='macro')
        self.test_accuracy_micro = torchmetrics.classification.Accuracy(task="multiclass", num_classes = num_classes, average='micro')
    ...
    def forward(self, x):         
        ...
        # create branch list
        branch_list = [torch.cat((CLS_raw, embedded_raw), dim=1), torch.cat((CLS_image, embedded_image), dim=1)]
        # encode the sequences
        encoded = self.encoder(branch_list)
        # classify
        output = [self.Classifier[i](encoded[i][:,0,:]) for i in range(self.num_branches)]
    ...
    def on_validation_epoch_end(self):
        if (self.current_epoch % self.display_frequency == 0):
            ...
            self.logger.experiment.add_figure("Validation Confusion Matrix", fig, self.current_epoch)
            plt.close(fig)
        self.pre_conf_val = []
        self.lab_conf_val = []    
